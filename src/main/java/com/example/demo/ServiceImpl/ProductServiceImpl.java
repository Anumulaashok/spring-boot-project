...
//		
		Admin A1= admindao.findById(AdminId).orElseThrow(()-> new CategoryException("Admin Not Avail With the id -> "+AdminId));
		
		Category c1= categoryDao.findById(id).orElseThrow(()-> new CategoryException("Category Not Found"));
		
		System.out.println(c1.toString());
		
		Products p1= new Products(product.getName(), product.getUrl(), product.getDesciption(),product.getPrice(),product.isAvail());
	...
	public Products updateProduct(Integer ProductId, ProductDto product) throws ProductException {
		...
		Products p1= productsDao.findById(ProductId).orElseThrow(()-> new ProductException("NO Product found with the Id ->" + ProductId));
